	...
	public StatementFigure(Statement<?> statement) {
		...
		if (statement.getValue() != null) {
			...
			if (statement.getOldValue() != null) {
				...
				arrowLabel.setLocation(new Point(a.getBounds().right() + BORDER_MARGIN / 2, a.getBounds().getRight().y
						- arrowLabel.getSize().height / 2));
				this.add(arrowLabel);
				...
				bounds.setSize(b.getBounds().right() + BORDER_MARGIN, BORDER_MARGIN + Math.max(a.getBounds().bottom(), b
						.getBounds().bottom()));
				setBounds(bounds);
			} else {
				this.add(label);
